...
const productionBackupBranch=backup.slice(backup.indexOf('if(runtime.mode==="production-api")'),backup.indexOf("const columns"));
...
check("dashboard uses runtime data mode",dashboard.includes("runtime.isProduction")&&dashboard.includes('runtime.mode==="production-api"')&&dashboard.includes("D1 / R2 / Queue backend"));
check("dashboard has honest empty activity state",dashboardPanels.includes("No recent production activity"));
check("production reset hidden",settings.includes("runtime.isLocal&&")&&settings.includes("Reset local data")&&settings.includes("if(!runtime.isLocal) return"));
...
check("production backup recovery uses D1 Time Travel",productionBackupBranch.includes("D1 Time Travel")&&!productionBackupBranch.includes("platformOperationsStore.restoreBackup"));
check("production cleanup is server controlled",cleanup.includes('runtime.mode==="production-api"')&&cleanup.includes("Server-controlled retention")&&cleanup.includes("browser cleanup is disabled"));
...
check("Gaming vNext reset is local only",vnextCms.includes("runtime.isLocal&&")&&vnextCms.includes("gamingVNextStore.reset()"));
...
check("supplier browser integration is server controlled",supplierRepo.includes("Browser supplier API configuration")&&supplierRepo.includes("Supplier catalog sync simulation")&&supplierControl.includes("nextf.vnext.gaming.supplier-command.fazercards")&&supplierPage.includes("queueFazerCommand")&&!supplierPage.includes("FAZERCARDS_API_KEY")&&!supplierPage.includes("apiSecret"));
...
check("Software update simulator is gated",updates.includes("disabled={!updateCapability.available}")&&updates.includes("updateCapability.available&&licenseId&&softwareStore.checkUpdate"));
check("Software delivery simulator is gated",downloads.includes("disabled={!deliveryCapability.available}")&&downloads.includes("deliveryCapability.available&&licenseId&&releaseId&&softwareStore.issueDownload"));
