...
check('CMS environment exposes dedicated review credential',env.includes('GAMING_CMS_REVIEWS_TOKEN?: string'));
check('review list uses dedicated review credential',reviewBlock.includes('/v1/gaming/admin/reviews?status=')&&reviewBlock.includes('token:env.GAMING_CMS_REVIEWS_TOKEN'));
check('review decision uses dedicated review credential',reviewBlock.includes('/decision`,token:env.GAMING_CMS_REVIEWS_TOKEN'));
