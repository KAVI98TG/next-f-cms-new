...
check('exact provider amount is not formatted as USD',page.includes('exactProviderAmount')&&(page.includes('Send this exact amount')||page.includes('Amount that must reach FazerCards'))&&(page.includes('Copy amount')||page.includes('Copy exact amount'))&&!page.includes('money(fundingRecord.payment?.uniqueAmount'));
check('network fee and exact-arrival warning are explicit',(page.includes('The network fee is separate')||page.includes('network or withdrawal fee is separate'))&&(page.includes('must arrive at the address')||page.includes('must deliver exactly'))&&(page.includes('wrong network, address, memo/tag or amount')||page.includes('mismatch can cause permanent loss')));
