...
check("P1 baseline retained at V0.24+", major === 0 && minor >= 24 && lock.version === pkg.version && lock.packages?.[""]?.version === pkg.version && read("VERSION").trim() === pkg.version && version.includes(`APP_VERSION = "${pkg.version}"`));
...
check("production bootstrap awaits durable state", main.includes("await initializeDurableStorage()") && main.includes("failed closed"));
