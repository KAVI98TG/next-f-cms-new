...
export function LiveOperationsPage(){
  ...
  const submitFulfillmentRetry=async()=>{
    ...
    setActionBusy(true);
    try{await retryGamingFulfillment(retryJob.jobId);notify({title:"Fulfillment retry queued",description:`${snapshot.orders.find((order)=>order.orderId===retryJob.orderId)?.orderNumber??retryJob.orderId} is queued for fulfillment retry.`,tone:"success"});setRetryJob(null);await load();}
  ...
  const submitRiskDecision=async()=>{
    ...
    setActionBusy(true);
    try{await decideGamingRisk(riskReview.assessmentId,riskDecision,riskNote.trim()||undefined);notify({title:riskDecision==="release"?"Risk hold released":"Risk hold retained",description:`${riskReview.orderNumber} risk review was saved.`,tone:riskDecision==="release"?"success":"info"});setRiskReview(null);setRiskNote("");await load();}
  ...
  return <div className="page live-operations-page">
    <SectionHeader eyebrow="Gaming Store · Operations" title="Live Operations" description="Monitor payments, fulfillment, risk, refunds and notification queues." action={<Button onClick={()=>void load()} disabled={loading||actionBusy}><RefreshCw size={15}/>Refresh</Button>}/>
    {loading&&!snapshot?<StatePanel state="loading" title="Loading live Gaming operations" description="Loading orders, payments, fulfillment, risk, refunds and notifications."/>:error&&!snapshot?<StatePanel state="error" title="Live operations unavailable" description={error} action={<Button onClick={()=>void load()}><RefreshCw size={15}/>Retry</Button>}/>:snapshot?<>
      <div className="live-ops-status-grid">
        <button className="live-ops-stat" onClick={()=>{setTab("orders");setQuery("");setQueueFilter("all");}}><span><ShoppingBag size={15}/>Active orders</span><strong>{activeOrders}</strong></button>
        <button className={`live-ops-stat ${paymentAttention?"is-warning":""}`} onClick={()=>{setTab("payments");setQuery("");setQueueFilter(paymentAttention?"pending_review":"all");}}><span><CircleDollarSign size={15}/>Payment review</span><strong>{paymentAttention}</strong></button>
        <button className={`live-ops-stat ${fulfillmentAttention?"is-danger":""}`} onClick={()=>{setTab("fulfillment");setQuery("");setQueueFilter(fulfillmentAttention?"attention":"all");}}><span><PackageCheck size={15}/>Fulfillment</span><strong>{fulfillmentAttention}</strong></button>
        <button className={`live-ops-stat ${riskAttention?"is-warning":""}`} onClick={()=>{setTab("risk");setQuery("");setQueueFilter(riskAttention?"attention":"all");}}><span><ShieldCheck size={15}/>Risk holds</span><strong>{riskAttention}</strong></button>
        <button className={`live-ops-stat ${refundAttention?"is-warning":""}`} onClick={()=>{setTab("refunds");setQuery("");setQueueFilter(refundAttention?"active":"all");}}><span><RotateCcw size={15}/>Refunds</span><strong>{refundAttention}</strong></button>
        <button className={`live-ops-stat ${notificationAttention?"is-warning":""}`} onClick={()=>{setTab("notifications");setQuery("");setQueueFilter(notificationAttention?"attention":"all");}}><span><Mail size={15}/>Notifications</span><strong>{notificationAttention}</strong></button>
        <div className="live-ops-stat live-ops-stat--sales"><span><CircleDollarSign size={15}/>{snapshot.capabilities.finance?"Net sales":"Commerce events"}</span><strong>{snapshot.capabilities.finance?gamingLkr(snapshot.summary.netSalesLkr??snapshot.summary.grossSalesLkr??0):snapshot.events.length}</strong></div>
      </div>
