...
export function FinancePage(){
  ...
  if(loading&&!snapshot)return <div className="page"><SectionHeader eyebrow="Gaming Store" title="Finance & Reconciliation" description="Loading the live Gaming finance ledger."/><StatePanel state="loading" title="Loading finance ledger" description="Reading verified customer payments, refund records and checkout-time economics from the shared commerce store."/></div>;
  ...
  return <div className="page">
    <SectionHeader eyebrow="Gaming Store · P4" title="Finance & Reconciliation" description="Live customer collections, refunds, supplier economics and recoveries from the shared Gaming commerce ledger." action={<ExportCsvButton filename="next-f-gaming-finance" rows={snapshot.orders} columns={[{header:"Order",value:(row)=>row.orderNumber},{header:"Customer",value:(row)=>row.customer.email},{header:"Product",value:(row)=>row.productName},{header:"Status",value:(row)=>row.status},{header:"Payment provider",value:(row)=>row.payment.providerKey},{header:"Collected LKR",value:(row)=>row.payment.state==="verified"?row.amountLkr:0},{header:"Refunded LKR",value:(row)=>refundedAmount(row.orderId)},{header:"Net customer revenue LKR",value:(row)=>(row.payment.state==="verified"?row.amountLkr:0)-refundedAmount(row.orderId)},{header:"Supplier cost LKR",value:(row)=>n(row.economics?.supplierCostLkr)},{header:"Supplier recovery LKR",value:(row)=>recoveries(row.orderId).supplier},{header:"Gateway fee LKR",value:(row)=>n(row.economics?.gatewayFeeLkr)},{header:"Gateway fee recovery LKR",value:(row)=>recoveries(row.orderId).gateway},{header:"Estimated margin LKR",value:(row)=>orderMargin(row)}]}/>}/>
    <div className="metric-grid">
      <MetricCard label="Gross collected" value={gamingLkr(snapshot.summary.grossCollectedLkr??0)} detail="Verified customer payments" icon={CircleDollarSign}/>
      <MetricCard label="Completed refunds" value={gamingLkr(snapshot.summary.completedRefundsLkr??0)} detail={`${snapshot.refunds.filter((refund)=>refund.status==="completed").length} completed refunds`} icon={RotateCcw}/>
      <MetricCard label="Net sales" value={gamingLkr(snapshot.summary.netSalesLkr??0)} detail={`${gamingLkr(snapshot.summary.pendingRefundsLkr??0)} still pending`} icon={WalletCards}/>
      <MetricCard label="Estimated margin" value={gamingLkr(snapshot.summary.estimatedMarginLkr??0)} detail="After refunds and recorded recoveries" icon={BadgeDollarSign}/>
    </div>
    <div className="compact-metrics">
      <MetricCard label="Supplier cost" value={gamingLkr(snapshot.summary.supplierCostLkr??0)} detail={`${gamingLkr(snapshot.summary.supplierRecoveriesLkr??0)} recovered`} icon={WalletCards}/>
      <MetricCard label="Gateway fees" value={gamingLkr(snapshot.summary.gatewayFeesLkr??0)} detail={`${gamingLkr(snapshot.summary.gatewayFeeRecoveriesLkr??0)} recovered`} icon={CreditCard}/>
    </div>
