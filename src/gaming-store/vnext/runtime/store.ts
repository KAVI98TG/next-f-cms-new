...
export const gamingVNextStore = {
  ...
  updateProducts(ids: string[], patch: Partial<NextFGamingProduct>) { const selected = new Set(ids); const updatedAt = new Date().toISOString(); const rows = this.getProducts().map((row) => selected.has(row.id) ? { ...row, ...patch, updatedAt } : row); return write(KEYS.products, rows); },
  ...
  updateOffers(ids: string[], patch: Partial<NextFGamingOffer>) { const selected = new Set(ids); const updatedAt = new Date().toISOString(); const rows = this.getOffers().map((row) => selected.has(row.id) ? { ...row, ...patch, updatedAt } : row); return write(KEYS.offers, rows); },
  ...
  updateMappings(ids: string[], patch: Partial<SupplierOfferMapping>) { const selected = new Set(ids); const rows = this.getMappings().map((row) => selected.has(row.id) ? { ...row, ...patch } : row); return write(KEYS.mappings, rows); },
