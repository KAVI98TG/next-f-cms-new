...
export function CatalogVNextPage() {
  ...
  const productCols: DataTableColumn<NextFGamingProduct>[] = [
    { key: "select", header: selectHeader, render: (product) => <input type="checkbox" className="catalog-row-check" aria-label={`Select ${publicName(product)}`} checked={selectedIds.has(product.id)} onChange={(event) => toggleSelected(product.id, event.target.checked)} /> },
    { key: "product", header: "NEXT F product", render: (product) => <div className="entity-cell catalog-entity"><strong>{publicName(product)}</strong><small>/{product.slug} · {product.id}</small></div> },
    { key: "kind", header: "Kind", render: (product) => <GamingKindBadge kind={product.kind}/> },
    { key: "offers", header: "Offers", render: (product) => { const stats = productStats.get(product.id); return <div className="entity-cell"><strong>{stats?.offerCount ?? 0}</strong><small>{stats?.enabledOfferCount ?? 0} public</small></div>; } },
    { key: "routing", header: "Routing", render: (product) => { const stats = productStats.get(product.id); const routable = (stats?.routableOfferCount ?? 0) > 0; const noOffers = (stats?.offerCount ?? 0) === 0; return <div className="entity-cell"><Badge tone={routable ? "success" : product.enabled ? "danger" : "neutral"}>{routable ? "Routeable" : noOffers ? "No offers" : "No live route"}</Badge><small>{stats?.activeRouteCount ?? 0} active · {stats?.supplierCount ?? 0} supplier{(stats?.supplierCount ?? 0) === 1 ? "" : "s"}</small></div>; } },
    { key: "featured", header: "Featured", render: (product) => <Toggle checked={product.featured} onChange={(featured) => gamingVNextStore.updateProduct(product.id, { featured })} /> },
    { key: "enabled", header: "Public", render: (product) => <Toggle checked={product.enabled} onChange={(enabled) => gamingVNextStore.updateProduct(product.id, { enabled })} /> },
    { key: "action", header: "", render: (product) => <Button onClick={() => { setProductSaveError(""); setArtworkPreviewFailed(false); setSelectedProduct(product); }}>Edit</Button> },
  ];
  const offerCols: DataTableColumn<NextFGamingOffer>[] = [
    { key: "select", header: selectHeader, render: (offer) => <input type="checkbox" className="catalog-row-check" aria-label={`Select ${offer.name}`} checked={selectedIds.has(offer.id)} onChange={(event) => toggleSelected(offer.id, event.target.checked)} /> },
    { key: "offer", header: "Retail offer", render: (offer) => { const product = products.find((row) => row.id === offer.productId); return <div className="entity-cell catalog-entity"><strong>{product ? publicName(product) : "Unknown product"} · {offer.name}</strong><small>{offer.id} · {kindLabel(offer.kind)} · {offer.purchaseFields.length} customer fields</small></div>; } },
    { key: "price", header: "Retail price", render: (offer) => <div className="entity-cell"><strong>{lkr(offer.sellingPriceLkr)}</strong><small>{offer.pricingMode.replaceAll("_", " ")}</small></div> },
    { key: "validation", header: "Validation", render: (offer) => <Badge tone={offer.validation.supported ? "success" : "neutral"}>{offer.validation.supported ? "Preflight" : "Not required"}</Badge> },
    { key: "routes", header: "Routing", render: (offer) => { const offerMappings = mappingsByOffer.get(offer.id) ?? []; const active = offerMappings.filter((mapping) => mapping.enabled && mapping.availability.state === "available").length; return <div className="entity-cell"><Badge tone={active ? "success" : offer.enabled ? "danger" : "neutral"}>{active ? "Routeable" : "No live route"}</Badge><small>{active} active · {offerMappings.length} total</small></div>; } },
    { key: "enabled", header: "Public", render: (offer) => <Toggle checked={offer.enabled} onChange={(enabled) => gamingVNextStore.updateOffer(offer.id, { enabled })} /> },
    { key: "action", header: "", render: (offer) => <Button onClick={() => setSelectedOffer(offer)}>Edit</Button> },
  ];
  const mappingCols: DataTableColumn<SupplierOfferMapping>[] = [
    { key: "select", header: selectHeader, render: (mapping) => <input type="checkbox" className="catalog-row-check" aria-label={`Select route ${mapping.id}`} checked={selectedIds.has(mapping.id)} onChange={(event) => toggleSelected(mapping.id, event.target.checked)} /> },
    { key: "route", header: "Offer / supplier", render: (mapping) => { const offer = offers.find((row) => row.id === mapping.offerId); const product = products.find((row) => row.id === offer?.productId); return <div className="entity-cell catalog-entity"><strong>{product ? publicName(product) : "Unknown product"} · {offer?.name ?? mapping.offerId}</strong><small>{supplierLabel(mapping.supplierId)} · {mapping.externalOfferId ?? mapping.externalProductId ?? "dynamic"}</small></div>; } },
    { key: "priority", header: "Priority", render: (mapping) => <Badge tone={mapping.priority === 1 ? "info" : "neutral"}>#{mapping.priority}</Badge> },
    { key: "cost", header: "Supplier cost", render: (mapping) => <div className="entity-cell"><strong>{money(mapping.supplierCost, mapping.supplierCurrency)}</strong><small>{mapping.availability.stock == null ? "Live availability" : `${mapping.availability.stock} stock`}</small></div> },
    { key: "availability", header: "Availability", render: (mapping) => <Badge tone={mapping.availability.state === "available" ? "success" : mapping.availability.state === "unknown" ? "warning" : "danger"}>{mapping.availability.state}</Badge> },
    { key: "enabled", header: "Route", render: (mapping) => <Toggle checked={mapping.enabled} onChange={(enabled) => gamingVNextStore.updateMapping(mapping.id, { enabled })} /> },
    { key: "action", header: "", render: (mapping) => <Button onClick={() => setSelectedMapping(mapping)}>Edit</Button> },
  ];
