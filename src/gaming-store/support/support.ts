...
export type GamingSupportCase={
  ...
  orderContext:{productName:string;offerName:string;amountLkr:number;orderStatus:string;paymentState:string;fulfillmentState?:string;refundStates:string[];risk?:{level:string;state:string;score:number}};
};
export type GamingSupportMessage={messageId:string;caseId:string;orderId:string;visibility:'internal'|'customer';body:string;author:{kind:'staff';accountId:string};createdAt:string};
export type GamingSupportEvidence={evidenceId:string;caseId:string;orderId:string;kind:'external_reference'|'customer_reference'|'media_asset';label:string;reference:string;sensitivity:'general'|'support_private';addedAt:string;addedBy:string};
