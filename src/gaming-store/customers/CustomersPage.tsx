...
function CustomerMetric({label,value,detail,icon:Icon,active,onClick}:{label:string;value:string;detail:string;icon:LucideIcon;active?:boolean;onClick?:()=>void}){
  const content=<><div className="customer360-metric__top"><span>{label}</span><span className="customer360-metric__icon"><Icon size={16}/></span></div><strong>{value}</strong><small>{detail}</small></>;
...
export function CustomersPage(){
  ...
  return <div className="page customer360-page">
    ...
    <div className="customer360-metrics">
      <CustomerMetric label="Customers" value={String(summary.customers)} detail={`${summary.signedIn} signed in · ${summary.guests} guest`} icon={UsersRound} active={accountType==='all'&&lifecycle==='all'&&marketing==='all'} onClick={()=>{setAccountType('all');setLifecycle('all');setMarketing('all');}}/>
      <CustomerMetric label="Repeat customers" value={String(summary.repeat)} detail={`${percent(summary.repeat,summary.customers)} of customer base`} icon={Repeat2} active={lifecycle==='repeat'} onClick={()=>setLifecycle(lifecycle==='repeat'?'all':'repeat')}/>
      <CustomerMetric label="Marketing subscribed" value={String(summary.marketingSubscribed)} detail={`${percent(summary.marketingSubscribed,summary.customers)} explicit opt-in`} icon={MailCheck} active={marketing==='subscribed'} onClick={()=>setMarketing(marketing==='subscribed'?'all':'subscribed')}/>
      <CustomerMetric label="Net customer spend" value={gamingLkr(summary.netSalesLkr)} detail={`${summary.orders} recorded orders`} icon={CircleDollarSign}/>
    </div>
    <Card className="customer360-workspace">
      ...
      <DataTable rows={rows} columns={columns} getKey={(row)=>row.customerId} empty="No canonical customers match these filters."/>
    </Card>
