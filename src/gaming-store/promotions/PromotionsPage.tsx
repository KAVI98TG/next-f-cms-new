...
export function PromotionsPage(){
  ...
  const save=async()=>{for(const target of [0,1,3,4] as BuilderStep[])if(!validateStep(target))return;setBusy(true);try{editing?await updateGamingPromotion(draft as GamingPromotionDraft&{campaignId:string}):await createGamingPromotion(draft);notify({title:editing?'Campaign updated':'Campaign created',description:'Campaign saved.',tone:'success'});setOpen(false);setDraft(emptyDraft());setStep(0);await load();}catch(err){notify({title:'Campaign save failed',description:err instanceof Error?err.message:'Unable to save campaign.',tone:'danger'});}finally{setBusy(false);}};
  ...
  if(loading&&!snapshot)return <div className="page"><SectionHeader eyebrow="Gaming Store" title="Promotions & Campaigns" description="Loading promotions."/><StatePanel state="loading" title="Loading campaigns" description="Loading campaign state and redemption usage."/></div>;
  if(error&&!snapshot)return <div className="page"><SectionHeader eyebrow="Gaming Store" title="Promotions & Campaigns" description="Campaign control is temporarily unavailable."/><StatePanel state="error" title="Promotions unavailable" description={error} action={<Button onClick={()=>void load()}><RefreshCw size={15}/>Retry</Button>}/></div>;
  ...
  return <div className="page promotions-control-page">
    <SectionHeader eyebrow="Gaming Store" title="Promotions & Campaigns" description="Create and schedule coupons and automatic discounts within pricing safeguards." action={<div className="table-actions"><Button onClick={()=>void load()}><RefreshCw size={15}/>Refresh</Button><Button variant="primary" disabled={!snapshot.capabilities.manage} onClick={openCreate}><Plus size={15}/>Create campaign</Button></div>}/>

    <div className="promotion-overview-grid">
      <button className={`promotion-overview-card ${view==='active'?'is-active':''}`} onClick={()=>setView('active')}><span><Sparkles size={15}/>Active</span><strong>{summary.active}</strong></button>
      <button className={`promotion-overview-card ${view==='scheduled'?'is-active':''}`} onClick={()=>setView('scheduled')}><span><CalendarClock size={15}/>Scheduled</span><strong>{summary.scheduled}</strong></button>
      <button className={`promotion-overview-card ${view==='draft'?'is-active':''}`} onClick={()=>setView('draft')}><span><Pencil size={15}/>Draft</span><strong>{summary.draft}</strong></button>
      <button className="promotion-overview-card" onClick={()=>setView('all')}><span><RefreshCw size={15}/>Redemptions</span><strong>{snapshot.summary.redemptions}</strong></button>
      <div className="promotion-overview-card promotion-overview-card--value"><span><BadgeDollarSign size={15}/>Discount granted</span><strong>{gamingLkr(snapshot.summary.discountGrantedLkr)}</strong></div>
    </div>

    <Card className="promotion-workspace">
      <div className="promotion-workspace__header">
        <div><span>Campaign workspace</span><h3>Marketing campaigns</h3></div>
