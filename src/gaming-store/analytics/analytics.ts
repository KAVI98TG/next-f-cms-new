...
export type GamingAnalyticsAcquisition={source:string;medium:string;campaign:string;orders:number;verifiedOrders:number;fulfilledOrders:number;paidRate:number|null;grossCollectedLkr?:number;estimatedMarginLkr?:number};
...
export type GamingAnalyticsSnapshot={
  ...
  tracking:{status:'active'|'quiet'|'awaiting';lastEventAt?:string;eventCount:number;pageViews:number;productViews:number;searches:number;filterUses:number;offerSelections:number;supportOpens:number;landingPages:Array<{path:string;views:number}>};
  acquisition:GamingAnalyticsAcquisition[];
