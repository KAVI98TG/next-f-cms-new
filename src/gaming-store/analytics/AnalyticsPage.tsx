...
function AnalyticsStat({label,value,icon:Icon,tone='default'}:{label:string;value:string;icon:LucideIcon;tone?:'default'|'accent'|'success'|'warning'}){
...
export function GamingAnalyticsPage(){
  ...
  if(loading&&!snapshot)return <div className="page"><SectionHeader eyebrow="Gaming Store" title="Commerce Analytics" description="Loading commerce analytics."/><StatePanel state="loading" title="Loading analytics" description="Loading commerce events and performance data."/></div>;
  ...
  return <div className="page gaming-analytics-page">
    <SectionHeader eyebrow="Gaming Store" title="Commerce Analytics" description="Demand, conversion, delivery health and commercial performance." action={<div className="analytics-window" aria-label="Analytics window">{([7,30,90] as const).map((value)=><Button key={value} variant={days===value?'primary':'secondary'} onClick={()=>setDays(value)}>{value}d</Button>)}</div>}/>

    <div className="analytics-kpi-strip">
      <AnalyticsStat label="Orders" value={String(snapshot.business.orders)} icon={ShoppingBag} tone="accent"/>
      <AnalyticsStat label="View → order" value={rate(snapshot.funnel.overallViewToOrderRate)} icon={TrendingUp}/>
      <AnalyticsStat label="Paid → fulfilled" value={rate(fulfilledConversion)} icon={PackageCheck} tone="success"/>
      {snapshot.capabilities.finance?<AnalyticsStat label="Net sales" value={gamingLkr(snapshot.business.netSalesLkr??0)} icon={CircleDollarSign} tone="success"/>:<AnalyticsStat label="Refund rate" value={rate(snapshot.business.refundRate)} icon={RefreshCw} tone="warning"/>}
    </div>
    ...
    <Card className="analytics-tracking-panel">
      ...
      <div className="analytics-tracking-health">
        <div className={`analytics-tracking-state is-${snapshot.tracking.status}`}><span>{snapshot.tracking.status==='active'?'Active':snapshot.tracking.status==='quiet'?'Quiet':'Awaiting data'}</span><strong>{snapshot.tracking.lastEventAt?new Date(snapshot.tracking.lastEventAt).toLocaleString('en-LK'):'No storefront event yet'}</strong><small>{snapshot.tracking.eventCount} storefront events in this window</small></div>
        <AnalyticsStat label="Page views" value={String(snapshot.tracking.pageViews)} icon={MousePointerClick}/>
        <AnalyticsStat label="Searches" value={String(snapshot.tracking.searches)} icon={Search}/>
        <AnalyticsStat label="Offer selections" value={String(snapshot.tracking.offerSelections)} icon={Filter}/>
      </div>
    ...
    <div className="analytics-insight-grid">
      <Card className="analytics-panel analytics-panel--funnel">
        ...
        <div className="analytics-funnel analytics-funnel--steps">{snapshot.funnel.stages.map((stage,index)=><div className="analytics-funnel__row" key={stage.key}>
          ...
          <div className="analytics-funnel__label"><strong>{stage.label}</strong>{stage.conversionFromPrevious!==null?<small>{rate(stage.conversionFromPrevious)} from previous</small>:null}</div>
      ...
      <Card className="analytics-panel analytics-panel--trend">
        ...
        <div className="analytics-trend-summary">
          ...
          <div><span>View → fulfilled</span><strong>{rate(snapshot.funnel.overallViewToFulfilledRate)}</strong></div>
        </div>
      </Card>
    </div>
    ...
    <Card className="analytics-health-panel">
      ...
      <div className="analytics-health-grid">
        <AnalyticsStat label="Risk holds" value={String(snapshot.business.riskHeld)} icon={ShieldAlert}/>
        <AnalyticsStat label="Notification delivery" value={rate(snapshot.business.notificationDeliveryRate)} icon={MailCheck}/>
        <AnalyticsStat label="Payment verification" value={minutes(snapshot.business.avgPaymentVerificationMinutes)} icon={Clock3}/>
        <AnalyticsStat label="Fulfillment" value={minutes(snapshot.business.avgFulfillmentMinutesFromPayment)} icon={PackageCheck}/>
        {snapshot.capabilities.finance?<AnalyticsStat label="Completed refunds" value={gamingLkr(snapshot.business.refundsLkr??0)} icon={RefreshCw}/>:null}
        {snapshot.capabilities.finance?<AnalyticsStat label="Estimated margin" value={gamingLkr(snapshot.business.estimatedMarginLkr??0)} icon={BadgeDollarSign} tone="success"/>:null}
      </div>
    </Card>

    <Card className="analytics-table-card analytics-acquisition-card"><div className="operation-section__head"><div><span>Acquisition</span><h3>Campaign & source performance</h3></div><MousePointerClick size={18}/></div><DataTable rows={snapshot.acquisition} columns={acquisitionColumns} getKey={(row)=>`${row.source}:${row.medium}:${row.campaign}`} empty="No attributed orders have been recorded in this window."/></Card>
