...
export function WebsitePlatformPage() {
  ...
  const backendOperationColumns: DataTableColumn<ApiOperationDefinition>[] = [
    { key: "operation", header: "Operation", render: (row) => <div className="entity-cell"><strong>{row.name}</strong><small>{row.description}</small></div> },
  ...
  return <div className="page website-platform-page">
    <SectionHeader eyebrow="NEXT F Digital" title="Website Platform" description="Manage customer workspaces, websites, access and publishing." action={<div className="table-actions"><Button onClick={() => setLinkOpen(true)}><Link2 size={15}/>Link organization</Button><Button variant="primary" onClick={() => setWorkspaceOpen(true)}><Plus size={15}/>Request workspace</Button></div>} />

    <div className="compact-metrics website-platform-metrics">
      <MetricCard label="Customer workspaces" value={String(workspaces.length)} icon={Building2} />
      <MetricCard label="Memberships" value={String(memberships.filter((item) => item.status !== "revoked").length)} icon={BadgeCheck} />
      <MetricCard label="Managed sites" value={String(siteConnections.length)} icon={Globe2} />
      <MetricCard label="Contract" value={WEBSITE_CONTRACT_VERSION} icon={FileCheck2} />
    </div>
      ...
      <div className="website-platform-overview-grid">
        <Card className="website-platform-readiness">
          <div className="card-section-heading">
            <div><strong>Operational readiness</strong></div>
        ...
        <Card className="website-platform-quick-actions">
          <div className="card-section-heading"><div><strong>Quick actions</strong></div></div>
    ...
    {view === "workspaces" && <>
      <div className="website-platform-section-intro"><div><span>Tenancy operations</span><h3>Customer workspace provisioning</h3></div><Badge tone="neutral">{workspaces.length} workspaces</Badge></div>
      ...
      <div className="website-platform-section-intro website-platform-section-intro--compact"><div><span>Website delivery</span><h3>Managed sites & migration</h3></div></div>
    ...
    {view === "contracts" && <>
      <div className="compact-metrics">
        <MetricCard label="Contract pin" value={WEBSITE_CONTRACT_VERSION} icon={FileCheck2} />
        <MetricCard label="Registry source" value={registryReadiness.ready ? "Trusted" : "Unavailable"} icon={ShieldCheck} />
        <MetricCard label="Manifest evidence" value={String(manifestEvidence.length)} icon={Boxes} />
        <MetricCard label="Validation evidence" value={String(validationEvidence.length)} icon={BadgeCheck} />
      </div>
      <Card className="architecture-callout"><strong><ShieldCheck size={16}/> Registry trust</strong><p>{registryReadiness.reason}</p></Card>
    ...
    {view === "access-policy" && <>
      ...
      <Card className="table-card"><div className="card-section-heading"><div><strong>Canonical customer access projections</strong><p>Immutable resolver-derived maximums. Staff may restrict these further but cannot expand them.</p></div><MetricCard label="Resolved sites" value={String(canonicalAccessProjections.length)} icon={ShieldCheck}/></div><DataTable rows={canonicalAccessProjections} columns={[
        ...
        { key: "authority", header: "Authority", render: (row) => <span>{row.verificationAuthority}</span> },
      ]} getKey={(row) => row.id} empty="No validated customer access projection is available." /></Card>
    ...
    {view === "approvals-publishing" && <>
      <div className="compact-metrics">
        <MetricCard label="Observed resource heads" value={String(managedResourceHeads.length)} icon={RefreshCcw} />
        <MetricCard label="Open change requests" value={String(changeRequests.filter((row) => !["applied", "rejected", "cancelled"].includes(row.status)).length)} icon={FileCheck2} />
        <MetricCard label="Open publish requests" value={String(publishRequests.filter((row) => !["published", "rejected", "cancelled"].includes(row.status)).length)} icon={Globe2} />
        <MetricCard label="Execution receipts" value={String(applicationReceipts.length + publishingReceipts.length)} icon={BadgeCheck} />
      </div>
      <Card className="architecture-callout"><strong><ShieldCheck size={16}/> Revision conflict protection</strong><p>Changes based on an older site revision require review again.</p></Card>
      <Card className="table-card"><div className="card-section-heading"><div><strong>Customer change requests</strong><p>Proposal data remains separate from authoritative website state. Review decisions are immutable audit events; approval only authorizes a later adapter application.</p></div></div><DataTable rows={changeRequests} columns={changeRequestColumns} getKey={(row) => row.id} empty="No customer website change requests." /></Card>
      <Card className="table-card"><div className="card-section-heading"><div><strong>Publication governance</strong><p>Publishing is independent from editing and change approval. Staff-approval and direct-publish policies both require a separate execution receipt before the system records a revision as published.</p></div></div><DataTable rows={publishRequests} columns={publishRequestColumns} getKey={(row) => row.id} empty="No customer publication requests." /></Card>
    </>}

    {view === "backend-api" && <>
      <div className="compact-metrics">
        <MetricCard label="V1 API operations" value={String(backendReadiness.operationCount)} icon={Boxes} />
        <MetricCard label="Idempotent mutations" value={`${backendReadiness.idempotentCommandCount}/${backendReadiness.commandCount}`} icon={ShieldCheck} />
        <MetricCard label="Customer operations" value={String(backendReadiness.customerOperationCount)} icon={UserCheck} />
        <MetricCard label="Production transport" value={backendReadiness.productionTransportConnected ? "Connected" : "Not connected"} icon={Wrench} />
      </div>
            <Card className="table-card"><div className="card-section-heading"><div><strong>API operation registry</strong><p>Both Admin CMS and Customer Workspace clients must call registered queries/commands. Mutating commands are idempotent, audited and authorized server-side.</p></div><Badge tone="success">Fail closed</Badge></div><DataTable rows={BACKEND_API_OPERATIONS} columns={backendOperationColumns} getKey={(row) => row.name} empty="No backend operations registered." /></Card>
    ...
    {view === "public-site" && <>
      <div className="compact-metrics">
        <MetricCard label="First-party host" value={NEXTF_PUBLIC_HOST} icon={Globe2} />
        <MetricCard label="Published services" value={String(publicSiteReadiness.publishedServiceCount)} icon={Boxes} />
        <MetricCard label="Verified route surfaces" value={`${publicSiteReadiness.verifiedRouteCount}/${publicSiteReadiness.totalRouteSurfaceCount}`} icon={Link2} />
        <MetricCard label="Conversion receipts" value={String(publicConversionReceipts.length)} icon={Activity} />
      </div>
      <Card className="architecture-callout"><strong><ShieldCheck size={16}/> Public write protection</strong><p>Lead and demo submissions use server-side validation, idempotency and abuse protection.</p></Card>
      {!publicSiteReadiness.liveRouteWiringComplete && <Card className="architecture-callout"><strong><ShieldAlert size={16}/> Live-site wiring incomplete</strong><p>{publicSiteReadiness.blocker}</p></Card>}
    ...
    {view === "lifecycle" && <>
      <Card className="architecture-callout"><strong><ShieldAlert size={16}/> Site shutdown requires an explicit action</strong><p>Billing state does not automatically suspend or revoke a managed website.</p></Card>
      <div className="compact-metrics">
        <MetricCard label="Offboarding plans" value={String(offboardingPlans.length)} icon={Wrench} />
        <MetricCard label="Data exports" value={String(dataExports.length)} icon={FileCheck2} />
        <MetricCard label="Ownership records" value={String(assetOwnership.length)} icon={KeyRound} />
        <MetricCard label="Contract migrations" value={String(contractMigrations.length)} icon={RefreshCcw} />
      </div>
      <Card><div className="card-section-heading"><div><strong>Production acceptance ledger</strong></div><Badge tone={productionAcceptanceSummary.productionReleaseAllowed ? "success" : "warning"}>{productionAcceptanceSummary.productionReleaseAllowed ? "Release allowed" : `${productionAcceptanceSummary.criticalPending.length} critical gates open`}</Badge></div><div className="settings-rows">
