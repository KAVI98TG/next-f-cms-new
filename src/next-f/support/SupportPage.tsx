...
export function SupportPage() {
  ...
  return <div className="page">
    <SectionHeader eyebrow="NEXT F Digital" title="Support" description="Manage service requests linked to clients, projects and sites." action={<Button variant="primary" onClick={()=>setOpen(true)}><Plus size={15}/> New ticket</Button>}/>
    <div className="compact-metrics"><MetricCard label="Open" value={String(tickets.filter((t)=>t.status==="open").length)} icon={Headphones}/><MetricCard label="In progress" value={String(tickets.filter((t)=>t.status==="in_progress").length)} icon={TimerReset}/><MetricCard label="High priority" value={String(tickets.filter((t)=>["high","urgent"].includes(t.priority)&&!["resolved","closed"].includes(t.status)).length)} icon={AlertTriangle}/><MetricCard label="Resolved" value={String(tickets.filter((t)=>t.status==="resolved").length)} icon={CheckCircle2}/></div>
