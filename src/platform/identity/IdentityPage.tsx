...
export function IdentityPage() {
  ...
  return <div className="page">
    <SectionHeader eyebrow="Platform" title="NEXT F Accounts" description="Canonical identity records. Identity is separate from staff authorization, customer organization membership, service entitlement, and website permissions." action={<Button variant="primary" onClick={() => setOpen(true)}><Plus size={15}/>Create identity record</Button>} />
    <div className="compact-metrics">
      <MetricCard label="NEXT F Accounts" value={String(accounts.length)} detail={`${accounts.filter((account) => account.verificationState === "verified").length} verified`} icon={UserRoundCheck}/>
      <MetricCard label="Staff links" value={String(staffLinks.filter((link) => link.status === "active").length)} detail="Identity â†’ staff principal" icon={Link2}/>
      <MetricCard label="Customer memberships" value={String(memberships.filter((membership) => membership.status !== "revoked").length)} detail="Separate authorization bridge" icon={BadgeCheck}/>
    </div>
    <Card className="architecture-callout"><strong>Identity â‰  authorization</strong><p>Creating or verifying a NEXT F Account establishes identity only. It does not grant internal CMS access, Customer Workspace membership, site access, or website-management permissions. Staff authorization remains on Platform Users/Roles; customer authorization is established by explicit workspace membership and later entitlement/contract resolution.</p></Card>
    <Card className="architecture-callout"><strong>Legacy Shared Accounts remain separate</strong><p>The existing Shared Accounts view is a cross-business profile aggregation built from Digital, Gaming, and Software customer records. It is useful for commerce context but is not the authentication identity authority and is not migrated automatically into this directory.</p></Card>
    <Card className="table-card"><PageToolbar query={query} onQueryChange={setQuery} placeholder="Search NEXT F Accountsâ€¦"/><DataTable rows={filtered} columns={columns} getKey={(account) => account.id} empty="No NEXT F Accounts match this search."/></Card>
