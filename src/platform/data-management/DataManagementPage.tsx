...
export function DataManagementPage() {
  ...
  return <div className="page"><SectionHeader eyebrow="Platform" title="Data management" description={runtime.mode==="production-api"?"Export production CMS application data for controlled portability. Browser restore is disabled; production recovery uses D1 Time Travel.":"Export and restore local prototype CMS state through the durable storage boundary."} />
    <div className="compact-metrics"><Card><span><DatabaseBackup size={17}/>CMS records</span><strong>{recordCount}</strong></Card><Card><span><ShieldCheck size={17}/>Format</span><strong>JSON</strong></Card><Card><span>Schema</span><strong>V0.24</strong></Card><Card><span>Storage</span><strong>{getDurableStateStatus().mode === "production-api" ? "D1" : "Local adapter"}</strong></Card></div>
    <div className="dashboard-grid"><Card><SectionHeader title="Export application data" description="Creates one portable JSON bundle containing NEXT F CMS durable-state keys visible to the current staff principal."/><Button variant="primary" onClick={exportData}><Download size={16}/>Export data</Button></Card><Card><SectionHeader title={runtime.mode==="production-api"?"Production import restricted":"Restore local snapshot"} description={runtime.mode==="production-api"?"Production application-data import is intentionally unavailable from the browser. Use a reviewed maintenance workflow; use D1 Time Travel for disaster recovery.":"Validates an exported CMS backup first, then asks for confirmation before overwriting matching local durable-state keys."}/><input ref={inputRef} className="visually-hidden" type="file" accept="application/json,.json" onChange={(event)=>void inspectImport(event.target.files?.[0])}/><Button disabled={runtime.mode==="production-api"} onClick={()=>inputRef.current?.click()}><FileUp size={16}/>{runtime.mode==="production-api"?"Import disabled":"Import data"}</Button></Card></div>
    <Card className="architecture-callout"><div className="data-status"><Badge tone={tone}>{tone === "danger" ? "Import error" : tone === "success" ? "Complete" : "Ready"}</Badge><span>{message}</span></div><p>This export/import tool is for controlled data portability. Production disaster recovery still uses D1 Time Travel and restricted backup artifacts, not browser storage.</p></Card>
