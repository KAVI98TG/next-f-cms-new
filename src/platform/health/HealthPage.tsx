...
export function HealthPage() {
  ...
  return <div className="page"><SectionHeader eyebrow="Platform" title="System health" description="CMS diagnostics for the active runtime."/><div className="health-hero"><Card><span className="health-hero__icon"><Activity size={22}/></span><div><small>Application status</small><strong>Healthy</strong></div></Card><Card><span className="health-hero__icon"><Route size={22}/></span><div><small>Routes</small><strong>{allNavigation.length}</strong></div></Card><Card><span className="health-hero__icon"><UsersRound size={22}/></span><div><small>Unified accounts</small><strong>{accounts.length}</strong></div></Card><Card><span className="health-hero__icon"><WalletCards size={22}/></span><div><small>Payments</small><strong>{payments.length}</strong></div></Card></div><Card><div className="health-list">{checks.map((check)=><div className="health-row" key={check.label}><span className={`health-row__icon ${check.state==="healthy"?"is-good":"is-deferred"}`}>{check.label.includes("Search")?<Search size={17}/>:check.label.includes("repository")?<Database size={17}/>:check.label.includes("Permission")?<ShieldCheck size={17}/>:<CheckCircle2 size={17}/>}</span><span><strong>{check.label}</strong><small>{check.detail}</small></span><Badge tone={check.state==="healthy"?"success":"info"}>{check.state}</Badge></div>)}</div></Card></div>;
}
