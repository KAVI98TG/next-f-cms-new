...
export function HealthPage() {
  const accounts=commerceCenter.getAccounts(); const payments=commerceCenter.getPayments(); const search=buildGlobalSearchIndex(); const notifications=getOperationsNotifications(); const helpArticles=helpCenterStore.getArticles(); const helpRequests=helpCenterStore.getRequests();
  const checks=[
    ...
    {label:"Local repository",detail:`${platformStore.getUsers().length} platform staff records loaded from browser persistence.`,state:"healthy"},
    ...
    {label:"Production backend",detail:"Not connected during product-development stage.",state:"deferred"},
  ];
