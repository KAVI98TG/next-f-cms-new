...
export function AccountsPage() {
  ...
  return <div className="page">
    <SectionHeader eyebrow="Platform" title="Shared customer profiles" description="Customer profiles across Digital, Gaming and Software." action={<ExportCsvButton filename="next-f-shared-accounts" rows={filtered} columns={[{header:"Name",value:(row)=>row.name},{header:"Email",value:(row)=>row.email},{header:"Company",value:(row)=>row.company??""},{header:"Domains",value:(row)=>row.domains.join(" | ")},{header:"Linked profiles",value:(row)=>row.refs.length},{header:"LKR value",value:(row)=>row.lifetimeValueLkr},{header:"USD value",value:(row)=>row.lifetimeValueUsd}]}/>} />
    ...
    <Card className="architecture-callout"><strong>Customer profile only</strong><p>Customer profiles do not grant CMS access. Staff authentication is managed in NEXT F Accounts.</p></Card>
  </div>;
}
