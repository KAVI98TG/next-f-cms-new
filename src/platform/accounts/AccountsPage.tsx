...
export function AccountsPage() {
  ...
  return <div className="page">
    <SectionHeader eyebrow="Platform" title="Shared accounts" description="A normalized identity view across Digital clients, Gaming customers and Software customers. Business-specific profiles remain isolated while matching email identities are linked here." action={<ExportCsvButton filename="next-f-shared-accounts" rows={filtered} columns={[{header:"Name",value:(row)=>row.name},{header:"Email",value:(row)=>row.email},{header:"Company",value:(row)=>row.company??""},{header:"Domains",value:(row)=>row.domains.join(" | ")},{header:"Linked profiles",value:(row)=>row.refs.length},{header:"LKR value",value:(row)=>row.lifetimeValueLkr},{header:"USD value",value:(row)=>row.lifetimeValueUsd}]}/>} />
    ...
    <Card className="architecture-callout"><strong>Shared identity boundary</strong><p>This page does not flatten business data into one generic customer table. It creates a stable cross-business account view while Digital, Gaming and Software retain their own operational profiles.</p></Card>
  </div>;
}
