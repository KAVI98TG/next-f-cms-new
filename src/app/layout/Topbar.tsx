import { Bell, BookOpen, HelpCircle, Menu, Search, UserRound } from "lucide-react";
...
export function Topbar({ onMenu, onSearch, onGuide, onShortcuts }: { onMenu: () => void; onSearch: () => void; onGuide: () => void; onShortcuts:()=>void }) {
  ...
  return <header className="topbar"><div className="topbar__left"><button className="icon-button topbar__menu" onClick={onMenu} aria-label="Open navigation"><Menu size={20}/></button><div className="topbar__context"><span>Workspace</span><strong>{domain.label}</strong></div></div><div className="topbar__actions"><button className="command-trigger" onClick={onSearch} aria-label="Open global search"><Search size={17}/><span>Search anything</span><kbd>⌘ K</kbd></button><button className="icon-button" onClick={onShortcuts} aria-label="Open keyboard shortcuts"><HelpCircle size={18}/></button><button className="icon-button" onClick={onGuide} aria-label="Open page guide"><BookOpen size={18}/></button><button className="icon-button" onClick={()=>navigate("/platform/notifications")} aria-label={`${unread} unread notifications`}><Bell size={18}/>{unread>0&&<span className="notification-indicator"/>}</button><div className="user-chip" title={`${user.email} · ${user.permissions.length} permissions`}><span className="user-chip__avatar"><UserRound size={17}/></span><span><strong>{user.name}</strong><small>{user.role}</small></span></div></div></header>;
}
