...
export async function gamingSupportSnapshot(db:D1DatabaseLike,principal:Principal,supportConfigured:boolean){
  ...
  const cases=casesRaw.map((row:any)=>{const c=row.value||{};const order=orders.get(text(c.orderId))||{};const job=fulfillment.get(text(c.orderId));const refunds=refundsByOrder.get(text(c.orderId))||[];const risk=riskByOrder.get(text(c.orderId));return{
    ...
    orderContext:{productName:text(order.productName),offerName:text(order.offerName),amountLkr:num(order.amountLkr),orderStatus:text(order.status),paymentState:text(order.payment?.state),fulfillmentState:text(job?.state)||undefined,refundStates:refunds.map((r:any)=>text(r.status)).filter(Boolean),risk:risk?{level:text(risk.level),state:text(risk.state),score:Number(risk.score||0)}:undefined}
  };});
  const caseIds=new Set(cases.map((c:any)=>c.caseId));
  const messages=messagesRaw.map((row:any)=>row.value).filter((m:any)=>caseIds.has(text(m.caseId))).map((m:any)=>({messageId:text(m.messageId),caseId:text(m.caseId),orderId:text(m.orderId),visibility:text(m.visibility),body:text(m.body),author:{kind:'staff',accountId:text(m.author?.accountId)},createdAt:text(m.createdAt)}));
  const evidence=evidenceRaw.map((row:any)=>row.value).filter((e:any)=>caseIds.has(text(e.caseId))).map((e:any)=>({evidenceId:text(e.evidenceId),caseId:text(e.caseId),orderId:text(e.orderId),kind:text(e.kind),label:text(e.label),reference:text(e.reference),sensitivity:text(e.sensitivity)==='support_private'?'support_private':'general',addedAt:text(e.addedAt),addedBy:text(e.addedBy)}));
